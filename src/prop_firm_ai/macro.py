...
def _fetch_yahoo_options(symbol: str, expiration: int | None = None) -> dict[str, Any]:
    ...
    headers = {"User-Agent": "Mozilla/5.0 market-research-assistant/0.1"}
...
def _fetch_fred_series(series_id: str, label: str) -> dict[str, Any]:
    response = requests.get(FRED_CSV_URL, params={"id": series_id}, timeout=10, headers={"User-Agent": "market-research-assistant/0.1"})
    response.raise_for_status()
...
def _fetch_free_news_results(query: str) -> list[dict[str, Any]]:
    response = requests.get(
        ...
        timeout=10,
        headers={"User-Agent": "market-research-assistant/0.1"},
    )
    response.raise_for_status()
